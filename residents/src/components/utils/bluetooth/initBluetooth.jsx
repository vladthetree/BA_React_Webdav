	...
	async function searchBluetoothDevice() {
		...
		if (navigator.bluetooth) {
			...
			try {
				...
				const somedevice = await getDeviceFromStore(
					OBJECT_STORE_USER_DATA,
					"readonly",
					"device",
				);

        console.log(`SOME DEVICE is : ${somedevice}`)
	
				const BluetoothRemoteGATTServer = await somedevice.gatt.connect();
        console.log("ABC")
				const BluetoothRemoteGATTService =
				await BluetoothRemoteGATTServer.getPrimaryService(NORDIC_SERVICE);

				const BluetoothCharacteristic =
				await BluetoothRemoteGATTService.getCharacteristic(NORDIC_TX);
			//	await writeValueBLEmessage(command.connected, BluetoothCharacteristic);
				// somedevice.gatt.disconnect();
				// try {
				//   console.log("TRY disconnect")
				//   await BluetoothRemoteGATTServer.disconnect();
				// } catch (error) {
